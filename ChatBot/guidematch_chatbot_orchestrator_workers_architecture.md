...
## 0. 구현 현황 (v2 — 2026-06 적용)

설계상 "목표"였던 하이브리드 검색·멀티턴 재작성·평가 루프가 실제 코드로 적용되었습니다.

**실행 파이프라인** (`lib/chatbot/orchestrator.ts`):
`질문 이해 → 멀티턴 질의 재작성 → 하이브리드 검색(어휘+임베딩) → 재정렬 → 답변(LLM) → 평가·로깅`

- **임베딩 시맨틱 검색**: `gemini-embedding-001`(768차원)로 FAQ+사이트 코퍼스를 빌드 타임에 임베딩(`lib/chatbot/embeddings.json`, 텍스트 해시→벡터). 런타임엔 질문 1건만 임베딩해 인메모리 코사인 유사도 계산 → 기존 어휘 점수에 **가산(additive)**. 키·임베딩이 없으면 자동으로 어휘 검색만으로 폴백. (`lib/chatbot/embeddings.ts`, `rag.ts`)
- **멀티턴 질의 재작성**: 후속 질문("그건 며칠 걸려요?")만 골라 Gemini Flash로 독립형 검색어로 재작성. 단발성 질문은 LLM 호출 없이 통과. 실패 시 직전 발화 이어붙이기 휴리스틱. (`lib/chatbot/query-rewrite.ts`)
- **모델**: 기본값을 단종된 `gemini-2.0-flash` → **`gemini-2.5-flash`**(thinking 비활성)로 교체. 환경변수 `GEMINI_CHAT_MODEL`이 설정돼 있으면 그 값이 우선하므로 운영 환경에서 구 모델로 고정돼 있지 않은지 확인 필요.
- **평가 하니스**: `lib/chatbot/eval/golden-set.json`(100문항) + `scripts/eval-chatbot.ts`로 Top-1/3/5·MRR 측정.

**측정 결과** (골든셋 100문항, FAQ Top-k 적중률):

| | Top-1 | Top-3 | Top-5 | MRR |
|---|---|---|---|---|
| 어휘 검색만 (이전) | 26% | 33% | 37% | 0.30 |
| 하이브리드+재작성 (현재) | 57% | 82% | 88% | 0.70 |

**유지보수 (중요)**: FAQ CSV(`ChatBot/faq_data*.csv`)를 수정하면 반드시 임베딩을 재생성해야 한다.
```bash
npm run chatbot:build   # 코퍼스/FAQ 번들 + 임베딩 재생성
npm run chatbot:eval    # 검색 정확도 측정 (회귀 확인)
```
임베딩은 텍스트 해시로 캐시되므로 변경된 항목만 새로 호출한다. 신규 FAQ에 임베딩이 없으면 그 행만 어휘 검색으로 폴백한다.
